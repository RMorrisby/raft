...
class Taft
    ...
    TEMPLATE_PROJECT_RAW_NAME_REGEX = /(yyrawnameyy)/i # e.g. BLUE SKY # regex to use when matching some template text
    ...
    def self.install(lang = nil, debugging = false, dest = "", overwrite_ok = false, project_name = "", project_abbrev = "")
        ...
        if project_name.empty?
            puts "\nPlease enter the name of your project (e.g. BLUE SKY) :"
    ...
    def self.adjust_file_names(dest_base_folder, project_name, project_abbrev)
        ...
        pd "project_file_name_part : #{project_file_name_part}"
        pd "project_file_abbrev_part : #{project_file_abbrev_part}"
    ...
    def self.adjust_file_contents(dest_base_folder, project_name, project_abbrev)
        pd "Now in #{__method__}; #{dest_base_folder}; #{project_name}; #{project_abbrev}"
        raw_name = project_name # should have been entered in form like BLUE SKY
        project_class_name_part = project_name.gsub(/[\s-]+/, "_").downcase # TODO use .snakecase ?
        project_class_name_uppercase_part = project_name.camelcase
        project_class_abbrev_part = project_abbrev.gsub(/[\s-]+/, "_").delete("_").downcase # TODO use .snakecase ?
        
        pd "project_class_name_part : #{project_class_name_part}"
        pd "project_class_name_uppercase_part : #{project_class_name_uppercase_part}"
        pd "project_class_abbrev_part : #{project_class_abbrev_part}"
